  ...
  <div>
    ...
    <UCard class="mb-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <div>
          <label class="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Posting Status</label>
          <select
      ...
      <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
        <div class="flex items-center gap-2.5 flex-wrap">
          ...
          >
            ...
            <button
              class="ml-0.5 p-0.5 rounded-full hover:bg-amber-200 dark:hover:bg-amber-800 transition-colors"
              @click="filters.pendingOnly = false; loadTransactions()"
            >
              <UIcon name="i-heroicons-x-mark" class="w-3 h-3" />
            </button>
          </span>
        ...
        <table class="w-full min-w-[700px]">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            >
              ...
              <td class="px-6 py-3 text-sm text-gray-900 dark:text-gray-100 max-w-[240px]">
                <span class="block truncate" :title="transaction.description">{{ transaction.description }}</span>
                <p
                  v-if="showPendingTransferNote(transaction)"
                  class="text-[11px] text-amber-600 dark:text-amber-300 mt-0.5"
                >
                  Pending at {{ transaction.account?.institution || transaction.account?.name || 'bank' }}
                </p>
              </td>
...
const filters = ref({
  ...
  pendingOnly: route.query.pendingOnly === 'true',
...
async function loadTransactions() {
  ...
  try {
    ...
    if (filters.value.pendingOnly) params.pendingOnly = 'true'
...
function showPendingTransferNote(transaction: { isPending?: boolean | null, type?: string | null }) {
...
function clearFilters() {
  filters.value = { search: null, categoryId: null, merchantId: null, merchantName: null, amountSign: null, uncategorizedOnly: false, pendingOnly: false, purchasedBy: null, type: null, accountId: null, date: null, startDate: null, endDate: null }
  offset.value = 0
  loadTransactions()
}
...
function exportCsv() {
  ...
  if (filters.value.pendingOnly) params.set('pendingOnly', 'true')
